...
public class RemoveItemTests : IClassFixture<DbTest>
{
    private readonly DbTest _dbTest;

    public RemoveItemTests(DbTest dbTest)
    {
        _dbTest = dbTest;
    }
    ...
    [Fact]
    public async Task Handle_RemovesOneItemFromCart_WhenQuantityIsGreaterThanOne()
    {
        // Arrange
        ...
        var command = new RemoveItem.Request(cart.Id, 1);
    ...
    [Fact]
    public async Task Handle_RemovesEntireItem_WhenQuantityIsOne()
    {
        // Arrange
        using var context = _dbTest.CreateContext();
        var cart = CreateTestCart();
    ...
    public async Task Handle_ReturnsFailure_WhenCartNotFound()
    {
        ...
        var handler = new RemoveItem.Handler(context);
        var command = new RemoveItem.Request(nonExistentCartId, 1);
    ...
    public async Task Handle_ReturnsFailure_WhenItemNotInCart()
    {
        ...
        var handler = new RemoveItem.Handler(context);
        var command = new RemoveItem.Request(cart.Id, 999); // Non-existent item
    ...
    [Fact]
    public async Task Handle_OnlyRemovesSpecifiedItem()
    {
        // Arrange
        using var context = _dbTest.CreateContext();
        var cart = CreateTestCart();
        ...
        Assert.Equal(1, pizzaItem.Count); // Decreased from 2
        Assert.Equal(2, burgerItem.Count); // Unchanged
        Assert.Equal(1, sodaItem.Count);   // Unchanged
    }
    ...
    public async Task Handle_SavesChangesToDatabase()
    {
        ...
        var item = Assert.Single(reloadedCart!.Items);
    ...
    public async Task Handle_RemovesMultipleItemsSequentially()
    {
        ...
        // Act - Remove three times
        var result1 = await handler.Handle(new RemoveItem.Request(cart.Id, 1), CancellationToken.None);
        var result2 = await handler.Handle(new RemoveItem.Request(cart.Id, 1), CancellationToken.None);
        var result3 = await handler.Handle(new RemoveItem.Request(cart.Id, 1), CancellationToken.None);
    ...
    public async Task Handle_WithDifferentItemSkus_RemovesCorrectItem()
    {
        ...
        var handler = new RemoveItem.Handler(context);
        var command = new RemoveItem.Request(cart.Id, 200);
    ...
    {
        // Assert
        var exception = Assert.Throws<ArgumentNullException>(() => new RemoveItem.Handler(null!));
        Assert.Equal("db", exception.ParamName);
    }

    [Fact]
    public async Task Handle_UpdatesCartItemPrice_WhenPriceMatches()
    {
        // Arrange
        using var context = _dbTest.CreateContext();
        var cart = CreateTestCart();
        ...
        Assert.Equal(10.00m, item.Sum); // 1 * 10.00
    }

    [Fact]
    public async Task Handle_CalculatesCorrectSum_AfterRemoval()
    {
        // Arrange
        using var context = _dbTest.CreateContext();
        var cart = CreateTestCart();
        
        cart.AddItem(1, "Pizza", 12.50m);
        cart.AddItem(1, "Pizza", 12.50m);
        cart.AddItem(1, "Pizza", 12.50m);
        ...
        var item = Assert.Single(updatedCart!.Items);
        Assert.Equal(2, item.Count);
        Assert.Equal(25.00m, item.Sum); // 2 * 12.50
    }
}