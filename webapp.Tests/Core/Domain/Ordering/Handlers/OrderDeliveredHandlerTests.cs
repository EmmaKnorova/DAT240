...
public class OrderDeliveredHandlerTests : IClassFixture<DbTest>
{
    private readonly DbTest _dbTest;

    public OrderDeliveredHandlerTests(DbTest dbTest)
    {
        _dbTest = dbTest;
    }
    ...
    public void Constructor_WithNullContext_ThrowsArgumentNullException()
    {
        ...
        var exception = Assert.Throws<ArgumentNullException>(() => 
            new OrderDeliveredHandler(null!, mockNotificationService.Object));
    ...
    public void Constructor_WithNullNotificationService_ThrowsArgumentNullException()
    {
        ...
        // Act & Assert
        var exception = Assert.Throws<ArgumentNullException>(() => 
            new OrderDeliveredHandler(context, null!));
        Assert.Equal("notificationService", exception.ParamName);
    }

    [Fact]
    public async Task Handle_SendsDeliveredNotificationToCustomer()
    {
        // Arrange
        using var context = _dbTest.CreateContext();
        var mockNotificationService = new Mock<INotificationService>();
        ...
        var handler = new OrderDeliveredHandler(context, mockNotificationService.Object);
        var notification = new OrderDelivered(order.Id);
        ...
        mockNotificationService.Verify(
            service => service.SendOrderDeliveredNotification(customer.Id, order.Id),
            Times.Once
        );
    }
    ...
    public async Task Handle_WithNonExistentOrderId_DoesNotSendNotification()
    {
        ...
        var notification = new OrderDelivered(nonExistentOrderId);
        ...
        // Assert
        mockNotificationService.Verify(
            service => service.SendOrderDeliveredNotification(It.IsAny<Guid>(), It.IsAny<Guid>()),
            Times.Never
        );
    }

    [Fact]
    public async Task Handle_WithSubmittedOrder_SendsNotification()
    {
        // Arrange
        using var context = _dbTest.CreateContext();
        var mockNotificationService = new Mock<INotificationService>();
    ...
    public async Task Handle_SendsNotificationWithCorrectCustomerId()
    {
        ...
        // Assert
        mockNotificationService.Verify(
            service => service.SendOrderDeliveredNotification(
    ...
    public async Task Handle_CanBeCalledMultipleTimes()
    {
        ...
        var order1 = CreateTestOrder(customer1, Status.On_the_way);
        var order2 = CreateTestOrder(customer2, Status.On_the_way);
        ...
        var notification1 = new OrderDelivered(order1.Id);
        await handler.Handle(notification1, CancellationToken.None);

        // Act - Handle second order
        var notification2 = new OrderDelivered(order2.Id);
        await handler.Handle(notification2, CancellationToken.None);

        // Assert
        mockNotificationService.Verify(
            service => service.SendOrderDeliveredNotification(customer1.Id, order1.Id),
            Times.Once
        );
        mockNotificationService.Verify(
            service => service.SendOrderDeliveredNotification(customer2.Id, order2.Id),
            Times.Once
        );
    }
    ...
    public async Task Handle_OnlyNotifiesSpecifiedOrder()
    {
        ...
        var mockNotificationService = new Mock<INotificationService>();
        var customer = CreateTestUser();
        var order1 = CreateTestOrder(customer, Status.On_the_way);
        var order2 = CreateTestOrder(customer, Status.On_the_way);
        ...
        var handler = new OrderDeliveredHandler(context, mockNotificationService.Object);
        var notification = new OrderDelivered(order1.Id);
        ...
        mockNotificationService.Verify(
            service => service.SendOrderDeliveredNotification(customer.Id, order1.Id),
            Times.Once
        );
        mockNotificationService.Verify(
            service => service.SendOrderDeliveredNotification(customer.Id, order2.Id),
            Times.Never
        );
    }
    ...
    public async Task Handle_IncludesCustomerInQuery()
    {
        ...
        using var context = _dbTest.CreateContext();
        var mockNotificationService = new Mock<INotificationService>();
        var customer = CreateTestUser("Jane Smith", "jane@example.com");
        var order = CreateTestOrder(customer, Status.On_the_way);
