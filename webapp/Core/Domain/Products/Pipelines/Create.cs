...
public class Create
{
    public record Request(string Name, string Description, decimal Price, int Cooktime, string? ImagePath = null) : IRequest<Response>;
    ...
    public class Handler : IRequestHandler<Request, Response>
    {
        ...
        public async Task<Response> Handle(Request request, CancellationToken cancellationToken)
        {
            ...
            if (errors.Length > 0)
            {
                return new Response(Success: false, createdItem: null, Errors: errors);
            }

            _db.FoodItems.Add(foodItem);
            await _db.SaveChangesAsync(cancellationToken);

            return new Response(Success: true, createdItem: foodItem, Errors: Array.Empty<string>());
        }
    }
}